  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      <div style={{ background: card, borderRadius: 12, border: `1px solid ${bdr}`,
          overflow: 'hidden' }}>
        <div style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: `1px solid ${bdr}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {isSafe
      ...
      <div style={{ background: card, borderRadius: 12, border: `1px solid ${bdr}`, overflow: 'hidden' }}>
        <button
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: sub }}>確率的シミュレーション</span>
      ...
      <div style={{ background: card, borderRadius: 12, padding: '16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: secLifePlan ? 12 : 0 }}>
          <button onClick={() => setSecLifePlan(v => !v)} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: sub }}>シナリオ設定</span>
      ...
      {(recentMonthlyAverages || monthlyGapImpact || incomeGrowthEstimate !== null) && (
        <div style={{ background: card, borderRadius: 12, border: `1px solid ${bdr}` }}>
          ...
          {/* --- C: 貯蓄率バナー（常時表示・折りたたみなし）--- */}
          {savingsRateData ? (
            <div style={{
              borderRadius: 12, overflow: 'hidden',
              background: darkMode
                ? `linear-gradient(120deg, ${savingsRateData.grade.color}18 0%, ${card} 60%)`
                : `linear-gradient(120deg, ${savingsRateData.grade.color}10 0%, ${card} 60%)`,
              border: `1.5px solid ${savingsRateData.grade.color}40`,
              padding: '14px 16px',
            }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <div>
                  <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', color: sub, marginBottom: 4 }}>貯蓄率</p>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 40, fontWeight: 800, color: savingsRateData.grade.color, lineHeight: 1 }}>{savingsRateData.latest}%</span>
                    <span style={{ fontSize: 12, fontWeight: 700, color: savingsRateData.grade.color, padding: '3px 10px', borderRadius: 20, background: `${savingsRateData.grade.color}20` }}>{savingsRateData.grade.label}</span>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontSize: 10, color: sub, marginBottom: 4 }}>3ヶ月平均</p>
                  <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 22, fontWeight: 700, color: sub }}>{savingsRateData.avg}%</p>
                </div>
              </div>
              <div style={{ height: 7, borderRadius: 4, background: darkMode ? '#1a1a1a' : '#efefef', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${Math.min(savingsRateData.latest, 100)}%`, borderRadius: 4, background: savingsRateData.grade.color, transition: 'width 0.6s ease' }} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 5 }}>
                <span style={{ fontSize: 10, color: darkMode ? '#333' : '#ccc' }}>0%</span>
                <span style={{ fontSize: 10, color: darkMode ? '#444' : '#bbb' }}>目標 20%</span>
                <span style={{ fontSize: 10, color: darkMode ? '#444' : '#bbb' }}>FIRE圏 50%</span>
              </div>
            </div>
          ) : (
            <div style={{ padding: '14px 16px', borderRadius: 12, background: card, border: `1px solid ${bdr}` }}>
              <p style={{ fontSize: 12, color: sub, textAlign: 'center' }}>📊 先月の振り返りを完了すると貯蓄率が表示されます</p>
            </div>
          )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, zIndex: 60 }}>
          <div style={{ background: card, borderRadius: 12, padding: 20, maxWidth: 360, width: '100%' }}>
            ...
            <a href={shareUrl} download="lifeplan.png" style={{
              ...
              padding: 12, background: '#0A84FF', borderRadius: 12, color: '#fff',
