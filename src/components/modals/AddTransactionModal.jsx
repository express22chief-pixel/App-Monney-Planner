 ...
 * 取引追加モーダル。
 * 支出 / 収入 / チャージ の3タイプに対応。
 * 支払方法: クレジット / 現金 / ウォレット（PayPay残高・Suica・PASMO等）
 */
...
export default function AddTransactionModal(props) {
  ...
  const [chargeFrom, setChargeFrom]       = useState('cash'); // 'cash' | 'credit'
  ...
  const handleAddCharge = () => {
    ...
    addCharge({
      date:       chargeDate,
      amount:     chargeAmount,
      walletId:   chargeWalletId,
      fromMethod: chargeFrom,
      fromCardId: chargeFrom === 'credit' ? chargeCardId : null,
      memo:       chargeMemo,
    });
                ...
                <button key={id}
                  onClick={() => {
                    if (id === 'charge') {
                      ...
                      setChargeMode(false);
                      setNewTransaction({ ...newTransaction, type: id, paymentMethod: id === 'expense' ? (newTransaction.paymentMethod || 'credit') : undefined });
                    }
                  }}
                  className="flex-1 py-2 rounded-xl font-bold text-sm transition-all duration-200"
                  style={{
                    backgroundColor: active ? color : (darkMode ? '#1C1C1E' : '#f5f5f5'),
                    color: active ? '#fff' : (darkMode ? '#d4d4d4' : '#737373'),
            ...
            <div className="space-y-2.5">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(wallets || []).map(w => (
                    <button key={w.id}
                      onClick={() => setChargeWalletId(String(w.id))}
                      className="px-3 py-1.5 rounded-full text-xs font-semibold transition-all"
                      style={{
                        backgroundColor: String(chargeWalletId) === String(w.id) ? w.color || theme.accent : (darkMode ? '#1C1C1E' : '#f5f5f5'),
                        color: String(chargeWalletId) === String(w.id) ? '#fff' : (darkMode ? '#d4d4d4' : '#737373'),
                      }}
                    >
                      {w.icon} {w.name}
                    </button>
                ...
                {wallets?.length === 0 && (
                  <p className={`text-xs ${theme.textSecondary}`}>設定 → 電子マネーでウォレットを追加してください</p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {[{ key: 'cash', label: '💵 現金・銀行口座' }, { key: 'credit', label: '💳 クレジット' }].map(({ key, label }) => (
                    <button key={key}
                      onClick={() => setChargeFrom(key)}
                      className="flex-1 py-2 rounded-xl text-xs font-semibold transition-all"
                      style={{
                        backgroundColor: chargeFrom === key ? theme.accent : (darkMode ? '#1C1C1E' : '#f5f5f5'),
                        color: chargeFrom === key ? '#fff' : (darkMode ? '#d4d4d4' : '#737373'),
                      }}
                    >{label}</button>
                  ))}
                </div>
                ...
                {chargeFrom === 'credit' && creditCards.length === 0 && (
                  <p className={`text-xs ${theme.textSecondary} mt-1`}>クレジットカードを先に設定に追加してください</p>
                )}
              </div>
              ...
              <div className={`rounded-xl px-4 py-3 ${darkMode ? 'bg-neutral-800/80 border border-neutral-700' : 'bg-neutral-50 border border-neutral-200'}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className={`flex-1 bg-transparent text-2xl font-bold tabular-nums ${theme.text} focus:outline-none`}
                    style={{ color: '#FF9F0A' }}
                  />
                </div>
              </div>
              ...
              <button
                ...
                disabled={!chargeAmount || !chargeWalletId}
                className="w-full py-3 rounded-xl font-semibold text-white transition-all duration-200"
                style={{
                  backgroundColor: (!chargeAmount || !chargeWalletId) ? (darkMode ? '#2a2a2a' : '#e5e7eb') : '#FF9F0A',
                  color: (!chargeAmount || !chargeWalletId) ? (darkMode ? '#525252' : '#9ca3af') : '#fff',
                }}
              >
                ⚡ チャージを記録する
              </button>
          ...
          {!chargeMode && (
            <div className="space-y-2">
                    ...
                    {PAYMENT_METHODS.map(({ key, label }) => (
                      <button key={key}
                        onClick={() => setNewTransaction({
                          ...newTransaction,
                          paymentMethod: key,
                          cardId:   key === 'credit' ? (newTransaction.cardId || (creditCards[0]?.id)) : null,
                          walletId: key === 'wallet' ? (newTransaction.walletId || (wallets?.[0]?.id ? String(wallets[0].id) : null)) : null,
                        })}
                    ...
                    <div className="flex gap-1.5 flex-wrap">
                      {wallets.map(w => (
                        <button key={w.id}
                          ...
                          className="px-2.5 py-1 rounded-lg text-xs font-semibold transition-all"
                          style={{
                            backgroundColor: (newTransaction.walletId || String(wallets[0]?.id)) === String(w.id) ? (w.color || theme.accent) : (darkMode ? '#2a2a2a' : '#f0f0f0'),
                            color: (newTransaction.walletId || String(wallets[0]?.id)) === String(w.id) ? '#fff' : (darkMode ? '#d4d4d4' : '#737373'),
                          }}
              ...
              <div className={`rounded-xl px-4 py-3 ${darkMode ? 'bg-neutral-800/80 border border-neutral-700' : 'bg-neutral-50 border border-neutral-200'}`}>
                ...
                {newTransaction.amount && (
                  <p className={`text-xs ${theme.textSecondary} mt-1 tabular-nums`}>{Number(newTransaction.amount).toLocaleString()} 円</p>
                )}
              </div>
              ...
              {/* メモ */}
              <textarea rows={2} placeholder="メモ（任意）"
                ...
                <div className={`rounded-xl overflow-hidden border ${darkMode ? 'border-neutral-700' : 'border-neutral-200'}`}>
                  ...
                  {newTransaction.isSplit && (
                    <div className={`px-3 pb-3 pt-2 space-y-2 ${darkMode ? 'bg-neutral-800/50' : 'bg-blue-50/50'}`}>
                        ...
                        {newTransaction.amount && newTransaction.splitMembers.length > 0 && (
                          <button
                            onClick={() => {
                              ...
                              const n = newTransaction.splitMembers.length + 1;
                              const perPerson = Math.floor(total / n);
                              setNewTransaction({ ...newTransaction, splitMembers: newTransaction.splitMembers.map(m => ({ ...m, amount: String(perPerson) })) });
                            }}
                            className={`text-xs px-2.5 py-1 rounded-lg font-semibold shrink-0 ${darkMode ? 'bg-blue-900/40 text-blue-300' : 'bg-blue-100 text-blue-600'}`}
                          >÷ 均等割り</button>
                        )}
                      </div>
                        ...
                        {newTransaction.splitMembers.map((member, idx) => (
                          <div key={idx} className="flex gap-1.5 items-center">
                            ...
                            {newTransaction.splitMembers.length > 1 && (
                              <button
                                onClick={() => setNewTransaction({ ...newTransaction, splitMembers: newTransaction.splitMembers.filter((_, i) => i !== idx) })}
                      ...
                      <button
                        onClick={() => setNewTransaction({ ...newTransaction, splitMembers: [...newTransaction.splitMembers, { name: '', amount: '' }] })}
                        className={`w-full py-1.5 rounded-lg text-xs font-semibold border-dashed border-2 ${darkMode ? 'border-neutral-600 text-neutral-400' : 'border-neutral-300 text-neutral-400'}`}
                      >＋ 人を追加</button>
                      {(() => {
              ...
              {/* 追加ボタン */}
              <button
                onClick={() => { addTransaction(); if (newTransaction.amount && newTransaction.category) setShowAddTransaction(false); }}
